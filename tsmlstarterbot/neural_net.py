...
class NeuralNet(object):
    ...
    def __init__(self, cached_model=None, seed=None):
        ...
        with self._graph.as_default():
            ...
            #scope='bn' scale=True activation_fn=None
            flattened_frames = tf.reshape(self._features, [-1, PER_PLANET_FEATURES])
            flattened_framesBN = tf.contrib.layers.batch_norm(flattened_frames, center=True, scale=True, scope='bn')
            first_layer = tf.contrib.layers.fully_connected(flattened_framesBN, self.FIRST_LAYER_SIZE,)
            #first_layerBN = tf.contrib.layers.batch_norm(first_layer, center=True, scale=True, scope='bn')
            second_layer = tf.contrib.layers.fully_connected(first_layer, self.SECOND_LAYER_SIZE,normalizer_fn=tf.contrib.slim.batch_norm)
            third_layer = tf.contrib.layers.fully_connected(second_layer, self.THIRD_LAYER_SIZE,normalizer_fn=tf.contrib.slim.batch_norm)
            forth_layer = tf.contrib.layers.fully_connected(third_layer, self.FORTH_LAYER_SIZE,normalizer_fn=tf.contrib.slim.batch_norm)
            #forth_layerBN = tf.contrib.layers.batch_norm(first_layer, center=True, scale=True)
            keep_prob = tf.placeholder(tf.float32)
            forth_layer_drop = tf.nn.dropout(forth_layer, keep_prob)

            fifth_layer = tf.contrib.layers.fully_connected(forth_layer_drop, self.FIFTH_LAYER_SIZE,normalizer_fn=tf.contrib.slim.batch_norm)
            #sixth_layer = tf.contrib.layers.fully_connected(fifth_layer, self.SIXTH_LAYER_SIZE)
            #seven_layer = tf.contrib.layers.fully_connected(sixth_layer, self.SEVENTH_LAYER_SIZE)
            ...
            self._optimizer = tf.train.AdamOptimizer(learning_rate=1e-4,epsilon=0.1).minimize(self._loss)
